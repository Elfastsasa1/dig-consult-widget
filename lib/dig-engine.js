...
async function consultLLM({ topic, context, urgency, depth }) {
  ...
  try {
    ...
    console.error('[DIG Engine] Error status:', err.status);
    console.error('[DIG Engine] Error body:', JSON.stringify(err.error || err.body || {}).substring(0, 500));
    return mockResponse({ topic, context, urgency, depth }, `LLM Error: ${err.message} (status=${err.status})`);
  }
}
